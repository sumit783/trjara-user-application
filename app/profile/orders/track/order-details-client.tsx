  ...
  return (
    <div className="min-h-screen bg-background pb-28 animate-in fade-in duration-300">
      ...
      <div className="max-w-xl mx-auto w-full px-4 py-6 space-y-6">
        ...
        {/* Repayment Required Banner */}
        {order.payment?.method === 'ONLINE' && order.payment?.status?.toLowerCase() !== 'paid' && (
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-3xl p-5 shadow-sm space-y-3.5 animate-in fade-in duration-300">
            <div className="flex gap-3">
              <Info className="h-5 w-5 text-amber-600 flex-shrink-0 mt-0.5" />
              <div>
                <h4 className="text-xs font-black text-amber-800 uppercase tracking-wide">Repayment Required</h4>
                <p className="text-[11px] font-semibold text-amber-700/90 leading-normal mt-0.5">
                  The online payment for this order was unsuccessful or is pending. Please complete your payment to process the order.
                </p>
              </div>
            </div>
            <Button 
              onClick={() => handleRepayment(order._id, order.orderNumber)} 
              className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold h-11 rounded-xl text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
            >
              <CreditCard size={14} />
        ...
        <div className="bg-white border border-border/40 rounded-3xl p-6 shadow-lg space-y-4">
          ...
          <div className="border-t border-border/30 pt-4 flex flex-col xs:flex-row xs:items-center justify-between gap-3 text-xs font-bold">
            ...
            } px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wide self-start xs:self-center`}>
