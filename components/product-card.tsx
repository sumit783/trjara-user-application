...
export function ProductCard({
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl overflow-hidden h-full shadow-[0_4px_20px_rgb(0,0,0,0.06)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.14)] transition-all duration-300 group flex flex-col border border-gray-100 relative">
      ...
      {/* Content Container */}
      <div className="px-2 pb-2 sm:p-4 flex-1 flex flex-col justify-between">
        <div>
        ...
        <div className="mt-3 flex items-end justify-between gap-1 pt-2 border-t border-gray-50">
          ...
          <button
            onClick={() => onAddToCart({ id, name, price, category, image, rating })}
            className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl sm:rounded-2xl bg-black hover:bg-primary text-white flex items-center justify-center transition-colors duration-300 shadow-md group-hover:scale-105 active:scale-95 flex-shrink-0"
          >
