...
import { Search, MapPin, Bell, ChevronDown, Home as HomeIcon, Briefcase } from 'lucide-react';
        ...
        <div className="flex items-center gap-2">
          <Dialog>
            <DialogTrigger asChild>
              <button className="flex items-center gap-2 hover:bg-white/10 px-3 py-1.5 -ml-3 rounded-xl transition-all duration-200 text-left focus:outline-none focus:ring-1 focus:ring-white/40 group cursor-pointer animate-fade-in">
                <MapPin size={18} className="text-white flex-shrink-0 group-hover:scale-110 transition-transform duration-200 animate-bounce" />
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-white/80 font-semibold mb-0.5">Delivery to</p>
                  <div className="flex items-center gap-1">
                    <p className="text-sm font-bold text-white max-w-[180px] truncate">
                      {displayLabel}
                    </p>
                    <ChevronDown size={14} className="text-white/80 group-hover:text-white transition-colors" />
                  </div>
                </div>
              </button>
            </DialogTrigger>
